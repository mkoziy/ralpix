...
import { summarizeAssistantProgress, summarizePiToolCall, } from "./pi-subprocess.js";
void test("summarizePiToolCall includes the command for exec_command", () => {
    assert.equal(summarizePiToolCall("exec_command", { cmd: "npm run check" }), "exec_command npm run check");
});
void test("summarizePiToolCall falls back to a compact key/value preview", () => {
    assert.equal(summarizePiToolCall("open", { ref_id: "turn0search0", lineno: 120 }), "open ref_id=turn0search0 lineno=120");
});
void test("summarizeAssistantProgress strips the ralpix completion block", () => {
    assert.equal(summarizeAssistantProgress([
...
//# sourceMappingURL=pi-subprocess.test.js.map