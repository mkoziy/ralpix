...
import { buildStatusWidgetView, markTaskExecutionFinished, markTaskExecutionStarted, normalizePlanPathArg, withRalpixErrorHandling, } from "./index.js";
void test("normalizePlanPathArg strips a leading file-mention marker", () => {
    assert.equal(normalizePlanPathArg("@docs/plans/my-feature.md"), "docs/plans/my-feature.md");
});
...
void test("normalizePlanPathArg preserves scoped names that are not paths", () => {
    assert.equal(normalizePlanPathArg("@scope/package"), "@scope/package");
});
void test("withRalpixErrorHandling surfaces thrown errors via notify", async () => {
    const notices = [];
    await withRalpixErrorHandling(async () => await Promise.reject(new Error("boom")), (message, level) => {
...
void test("withRalpixErrorHandling leaves successful runs untouched", async () => {
    const notices = [];
    await withRalpixErrorHandling(async () => await Promise.resolve(), (message, level) => {
        notices.push(`${level}:${message}`);
    });
...
void test("buildStatusWidgetView renders review stages with live status detail", () => {
    ...
    const view = buildStatusWidgetView(state, [{ id: "task-1", title: "Task 1" }], 1, 0.034, new Map([["task-1", new Map([["opencode-go/deepseek-v4-pro", { input: 1000, output: 2000, cost: 0.01 }]])]]), new Map([["second-pass", new Map([["opencode-go/glm-5.1", { input: 3200, output: 800, cost: 0.024 }]])]]));
