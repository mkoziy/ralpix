...
function flushCurrentTask(state) {
...
function handleH1(line) {
    const match = RE_H1.exec(line);
    return match?.[1]?.trim() ?? null;
}
function handleH2(heading) {
...
function handleH3(line, state) {
    const match = RE_H3_TASK.exec(line);
    if (match === null)
        return;
    const numStr = match[1];
    const taskTitle = match[2];
    if (numStr === undefined || taskTitle === undefined)
        return;
...
function handleCheckbox(line, state) {
    ...
    if (done === undefined || text === undefined)
        return;
    const item = { text: text.trim(), done: done === "x" };
...
function collectLine(line, state) {
...
function inferTaskStatuses(tasks) {
...
export function parsePlan(filePath) {
    ...
    for (const line of lines) {
        ...
        if (RE_EMPTY.test(line) && state.section !== "task")
            continue;
        // Collect content
...
export function findNextPendingTask(plan) {
    return plan.tasks.find((t) => t.status === "pending") ?? null;
}
/** Update checkboxes in the plan file to reflect task status */
export function updatePlanTaskStatus(planPath, _taskId, taskTitle, status) {
