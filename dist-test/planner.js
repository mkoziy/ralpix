...
function buildPlannerEnv(piAgentDir) {
...
function buildPlanGenerationPrompt(basePrompt, round, clarifications, previousDraft, feedback) {
    const sections = [
        basePrompt,
        "",
        "## Runtime Override",
    ...
    if (round > 1 && previousDraft !== undefined) {
        sections.push("", "## Previous Draft", previousDraft, "", "## Revision Request", feedback ?? "Revise the draft.", "", "Return the full updated plan markdown only.");
    }
    return sections.join("\n");
}
function extractClarificationRequest(text) {
...
async function askClarification(ctx, req) {
...
function validatePlanDraft(content) {
...
function draftFileNameFromContent(content, fallbackDescription, createdAt) {
...
function nextAvailableDraftPath(plansDir, fileName, previousPath) {
...
function saveDraftFile(plansDir, description, content, createdAt, previousPath) {
    const nextPath = nextAvailableDraftPath(plansDir, draftFileNameFromContent(content, description, createdAt), previousPath);
...
async function runPlannerProcess(cwd, promptContent, round, config, launchConfig) {
...
async function reviewDraft(ctx, draftPath) {
    const reviewChoice = await ctx.ui.select(`Plan draft saved to ${draftPath}. What next?`, [
    ...
    if (typeof reviewChoice === "string" && reviewChoice.includes("Revise")) {
        const feedback = await ctx.ui.input("What changes would you like?", "Add more details, change approach, fix issues...");
...
// eslint-disable-next-line sonarjs/cognitive-complexity
export async function runPlanCreation(description, ctx, _pi, config) {
    ...
    for (let round = 1; round <= 5; round++) {
        ...
        for (const [launchIndex, launchConfig] of launchConfigs.entries()) {
            ...
            if (result.exitCode === 0)
                break;
            appendPlanCreationDebug(ctx.cwd, `round ${round}: launch ${String(launchIndex + 1)} failed exit=${String(result.exitCode)}`);
        }
        if (result?.exitCode !== 0) {
            ctx.ui.notify(`Plan creation failed in subprocess. See ${planCreationDebugFilePath(ctx.cwd)} and stderr in debug log.`, "error");
            return null;
        }
        ...
        appendPlanCreationDebug(ctx.cwd, `round ${round}: extracted draft ${draftStatus}`);
        ...
        if (draft.length === 0) {
            ctx.ui.notify(`Plan creation produced no draft. See ${planCreationDebugFilePath(ctx.cwd)}`, "error");
            return null;
        }
        ...
        for (;;) {
            ...
            if (review.action === "accept") {
                ...
                const execute = await ctx.ui.select("Plan created. What next?", ["▶ Execute plan now", "✓ Done — exit, run later"]);
