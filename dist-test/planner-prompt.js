export function buildPlanCreationPrompt(prompt, attempt = 1) {
    const lines = [
        prompt,
        "",
        "## Completion Contract",
    ...
    if (attempt > 1) {
        lines.push("", "## Retry Notice", "The previous attempt ended without submitting a draft.", "This time, you must either submit a draft with `ralpix_submit_plan_draft` or receive an explicit rejection from the user.");
    }
    return lines.join("\n");
}
export function planCreationAttemptConfigs() {
    return [
...
export function plannerLaunchConfigs() {
    return [
