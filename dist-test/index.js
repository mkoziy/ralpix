...
import { ProgressLogger, fmtTokens } from "./logger.js";
...
function createTokenLedger() {
    const map = new Map();
    function add(provider, model, usage) {
...
function recordUsage(usageById, id, provider, model, usage) {
    const key = `${provider}/${model}`;
    const perModel = usageById.get(id) ?? new Map();
...
function usageLinesFor(id, usageById) {
    ...
    if (perModel === undefined)
        return [];
    return [...perModel.entries()].map(([key, usage]) => `${key}  in ${fmtTokens(usage.input)}  out ${fmtTokens(usage.output)}  $${usage.cost.toFixed(3)}`);
}
...
function createInitialReviewState(externalReviewEnabled) {
    return {
        stages: [
...
function updateReviewStage(review, stageId, status, detail) {
    return {
...
export function buildStatusWidgetView(state, tasks, total, totalCost = 0, taskUsageById = new Map(), reviewUsageById = new Map()) {
...
export function normalizePlanPathArg(rawPath) {
...
export async function withRalpixErrorHandling(action, notify) {
    try {
        await action();
    }
    catch (error) {
...
export function markTaskExecutionStarted(state, taskId) {
    return {
        ...state,
        currentTaskId: taskId,
    };
}
export function markTaskExecutionFinished(state, taskId, success) {
...
// ---------------------------------------------------------------------------
export default function ralpixExtension(pi) {
    // ---- /ralpix command ----------------------------------------------------
    pi.registerCommand("ralpix", {
        ...
        handler: async (args, ctx) => withRalpixErrorHandling(async () => {
            ...
            if (trimmed.length === 0 || trimmed === "init") {
                const ok = await ctx.ui.confirm("Initialize ralpix?", "Create ~/.ralpix/ with default prompts, agents, and config?");
                if (ok !== true)
                    return;
                initRalpixHome();
                ctx.ui.notify("ralpix initialized — ~/.ralpix/ created with defaults", "success");
                return;
            }
    ...
    pi.registerTool({
        ...
        parameters: Type.Object({
            taskId: Type.Optional(Type.String({ description: "Task ID to mark done (uses current if omitted)" })),
        }),
    ...
    pi.on("session_start", (_event, ctx) => {
        ...
        if (state !== null && state.phase !== "complete" && state.phase !== "idle") {
            ...
            ctx.ui.notify(`ralpix: previous run of "${state.planTitle}" was interrupted (phase: ${state.phase}).\nResume with: ${resumeCmd}`, "warning");
...
// ---------------------------------------------------------------------------
async function runPlan(rawPath, ctx, pi) {
    ...
    ctx.ui.notify(`ralpix: "${plan.title}" — ${plan.tasks.length} tasks, ${pendingCount} pending`, "info");
    ...
    const taskUsageById = new Map();
    const reviewUsageById = new Map();
    const onUsage = (provider, model, usage) => {
    ...
    if (pendingCount > 0) {
        ...
        if (!allSuccess) {
            ctx.ui.notify(`ralpix: execution stopped — ${state.failedTasks.length} task(s) failed`, "error");
            state.phase = "idle";
            persistState(pi, state);
            updateStatusWidget(state, ctx, ledger, taskUsageById, reviewUsageById);
            return;
    ...
    try {
        await runReviewPipeline(ctx, pi, plan, config, logger, {
            ...
            onStageStart(stage, detail) {
                state.review = updateReviewStage(state.review ?? createInitialReviewState(config.externalReviewEnabled), stage, "active", detail);
                persistState(pi, state);
                updateStatusWidget(state, ctx, ledger, taskUsageById, reviewUsageById);
            },
            ...
            onStageFinish(stage, status, detail) {
                state.review = updateReviewStage(state.review ?? createInitialReviewState(config.externalReviewEnabled), stage, status, detail);
                persistState(pi, state);
                updateStatusWidget(state, ctx, ledger, taskUsageById, reviewUsageById);
            },
        });
    ...
    ctx.ui.notify(`ralpix: "${plan.title}" complete — ${done} done, ${failed} failed. Progress: ${logger.filePath}`, failed > 0 ? "warning" : "success");
...
function updateStatusWidget(state, ctx, ledger, taskUsageById = new Map(), reviewUsageById = new Map()) {
