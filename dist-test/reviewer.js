...
function detectDefaultBranch(cwd) {
    try {
        const remoteHead = execSync("git symbolic-ref refs/remotes/origin/HEAD --short", { cwd, encoding: "utf-8" }).trim();
...
export function buildReviewPrompt(promptContent, phase) {
    const lines = [
        promptContent,
        "",
        "## Completion Contract",
    ...
    switch (phase) {
        case "external": {
            lines.push("Use `Success: true` and put the exact review findings in `Summary`.", "If the review is clean, set `Summary` to exactly `NO ISSUES FOUND`.", "Use `Success: false` only if you cannot complete the review.");
            break;
        }
        case "eval": {
            lines.push("Use `Success: true` with a concise summary of what you evaluated and fixed.", "Include `EXTERNAL_REVIEW_DONE` in `Summary` when all findings are resolved.", "Use `Success: false` only if you cannot complete the evaluation.");
            break;
        }
        case "first":
        case "second": {
            lines.push("Use `Success: true` with a concise summary when the review pass completes.", "Use `Success: false` with the blocker or failure reason when you cannot complete the review.");
            break;
        }
    }
...
export function parseReviewSessionReport(text) {
...
async function runReviewSession(ctx, promptContent, phase, modelCfg, piAgentDir, includeEffort = true, onProgress, onUsage) {
    const result = await runPiSubprocessPrompt(ctx.cwd, buildReviewPrompt(promptContent, phase), modelCfg, includeEffort, 30 * 60 * 1000, createPiProgressHooks(onProgress, onUsage), piAgentDir);
...
async function runReviewProcess(ctx, promptName, config, plan, logger, defaultBranch, phase, includeEffort = true, extraVars, onUsage) {
...
async function runFirstReview(ctx, config, plan, logger, defaultBranch, hooks) {
    ...
    const result = await runReviewProcess(ctx, "review-first", config, plan, logger, defaultBranch, "first", true, undefined, hooks?.onUsage);
    if (result.success) {
        const msg = "COMPLETE";
...
async function runReviewLoop(ctx, config, plan, logger, defaultBranch, hooks) {
    ...
    for (let i = 0; i < maxIterations; i++) {
        ...
        const result = await runReviewProcess(ctx, "review-second", config, plan, logger, defaultBranch, "second", true, undefined, hooks?.onUsage);
        if (!result.success) {
            const msg = `ERROR: ${result.summary.slice(0, 200)}`;
            logger.logReview("loop", msg);
...
async function runExternalReviewLoop(ctx, config, plan, logger, defaultBranch, hooks) {
    ...
    const reviewerLabel = buildSessionModelChange(externalModelCfg)?.model ??
    ...
    const piAgentDir = resolvePiAgentDir(ctx.cwd, config);
    logger.logExternalReview("loop", `STARTED (reviewer: ${reviewerName}, max ${maxIterations} iterations, patience: ${patience})`);
    ...
    for (let i = 0; i < maxIterations; i++) {
        ...
        const reviewResult = await runReviewSession(ctx, reviewPrompt, "external", externalModelCfg, piAgentDir, true, undefined, hooks?.onUsage);
        ...
        const evalResult = await runReviewProcess(ctx, "external-eval", config, plan, logger, defaultBranch, "eval", true, { FINDINGS: findings }, hooks?.onUsage);
...
export async function runReviewPipeline(ctx, _pi, plan, config, logger, hooks) {
